...
刷新页面后仍必须能看到手动校准或候选确认后的可见状态。只在当前 UI session 中显示校准状态、刷新后丢失审计信息，视为 P17 blocker。
...
- 如果未来新增“辅助确认”类 action，也必须复用同一候选确认、领域校验、用户确认和审计保存路径；不得以 Agent 内部确认替代用户确认。

Negative acceptance:

- Agent 尝试直接提交最终坐标时，必须返回结构化错误或拒绝结果。
- Agent 尝试删除、隐藏或标记完成 pending place 时，必须返回结构化错误或拒绝结果。
...
- 详情页在目标尺寸下截断或不可滚动。
- 手动挪动没有取消路径。
- 手动挪动确认后刷新页面丢失校准状态或审计兜底。
- 图钉点击或缩放时闪烁。
- 筛选栏出现半截按钮。
- 分享图与当前筛选结果不一致。
- Agent 能绕过用户确认改坐标、删除或隐藏 pending place。
- 未使用真实数据组合进行验收。
