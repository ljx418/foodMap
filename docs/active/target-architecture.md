# FoodMap P19/P20-C/P21/P22/P23/P24/P25/P26/P27 Target Architecture
...
P26 is the latest accepted implementation baseline. P27 is the current mainland China public access planning stage. GitHub Pages is the accepted overseas/developer fixed URL baseline, but it is not the target production entry for mainland China users. Mainland-facing production must use the same static WebApp architecture on a domestic static host, with a stable HTTPS URL, no private preview token requirement, and the `mainland` build/verification profile. A custom mainland domain requires ICP filing and HTTPS before it can be called production-ready.

## 0F. P27 Mainland Public Access Target Architecture

P27 changes the host and acceptance chain, not the FoodMap business architecture. The app remains static assets plus browser-local data. The target architecture is:

```text
Mainland user or Mate70 browser
  -> stable public HTTPS URL without login or expiring preview token
  -> EdgeOne Pages, Tencent COS/CDN, Alibaba OSS/CDN, or domestic static server
  -> dist/ generated by npm run build:mainland or npm run build:edgeone
  -> index.html / assets/* / manifest.webmanifest / sw.js
  -> App hash router (#/map and #/share/:snapshotId)
  -> MapWorkspace / ShareView / ImportExportDialog / WebAppStatus
  -> Domain helpers and IndexedDB repositories
  -> .foodmap.json import/export for cross-device portability
  -> P27 verifier, browser smoke, Mate70 evidence, and final report
```

| Module | Responsibility | Concrete Entities | Status |
| --- | --- | --- | --- |
| `MainlandStaticHostProfile` | Define provider routes, domain/ICP/HTTPS/cache policy, free-first and fallback decisions, and paid-operation stop conditions | `docs/active/mainland-deployment-profile.md` | 需收口 |
| `MainlandBuildProfile` | Build static assets for domestic custom-domain root or configured subpath | `vite.config.ts`, `package.json` `build:mainland`, `build:edgeone`, `FOODMAP_MAINLAND_BASE_PATH`, `dist/` | 已实现，P27 需复验 |
| `EdgeOneDeployAdapter` | Deploy verified `dist/` through EdgeOne CLI when API token and project are available; support dry-run and token guardrails | `scripts/deploy_edgeone_pages.mjs`, `npm run deploy:edgeone`, `EDGEONE_PROJECT_NAME`, environment-only API token | 已实现，P27 需公网验收 |
| `MainlandDeploymentVerifier` | Verify local and remote static deployment without GitHub Pages `/foodMap/` assumptions | `scripts/verify_mainland_deployment.mjs`, `npm run verify:mainland:deployment`, `FOODMAP_MAINLAND_DEPLOY_URL` | 已实现，P27 需远端证据 |
| `PublicUrlEvidenceHarness` | Classify stable public URL, protected preview, GitHub Pages, LAN, HDC, and temporary tunnel; prevent false acceptance | `docs/active/evidence/p27/`, P27 browser smoke, P27 final report | 待新增 |
| `Mate70MainlandSmokeHarness` | Prove the stable mainland URL opens and completes P26 accepted main paths on Mate70 | HDC-assisted screenshot or recording workflow, Mate70 browser, P27 final report | 待新增 |
| `SecretRedactionGuard` | Keep API tokens, cookies, SecretKey values, and full protected-preview query tokens out of tracked docs and reports | Deployment docs, audit grep, script output policy, final report checklist | 需强化 |
| `LocalDataBoundary` | Preserve IndexedDB and `.foodmap.json` as the only user-data persistence and portability paths | IndexedDB repositories, import/export codec, PRD non-goals, `ShareView` | 已实现，P27 必须保持 |

P27 host state classification:

| Host State | May Count As | Must Not Count As |
| --- | --- | --- |
| GitHub Pages `https://ljx418.github.io/foodMap/` | Overseas/developer accepted baseline and P26 regression reference | Mainland production acceptance |
| Local `vite`, LAN IP, HDC reverse, or temporary tunnel | Development or diagnostic transport | Public access acceptance |
| EdgeOne protected preview with login or expiring query token | Deployment package smoke and browser package evidence | Stable mainland public URL |
| EdgeOne/COS/OSS/domestic static URL without login or expiring token | Candidate P27 public URL if HTTPS and remote verifier pass | Accepted final state before Mate70 evidence and final report |
| ICP-filed custom mainland domain with HTTPS | Preferred production target | Cloud sync, account, backend, or permanent share capability |

P27 write-path rule: P27 may change build/deploy/verify scripts, static-host docs, evidence scripts, and release reports only to make mainland static access repeatable and honest. It must not change IndexedDB data semantics, `.foodmap.json` schema, Agent write powers, coordinate confirmation rules, governance writes, or share snapshot semantics.

## 0E. Mainland China Static Deployment Architecture

FoodMap can serve mainland China users without adding a backend because the app is a static WebApp and user data remains in IndexedDB. This section is the deployment profile foundation for P27. The target deployment architecture changes only the host profile:
...
| Module | Responsibility | Concrete Entities | Status |
| --- | --- | --- | --- |
| `MainlandStaticHostProfile` | Define EdgeOne Pages, COS/CDN, OSS/CDN routes, ICP/HTTPS/domain requirements, cache policy, and provider options | `docs/active/mainland-deployment-profile.md` | 需收口 |
| `MainlandBuildProfile` | Build static assets for a custom-domain root path or configured subpath | `vite.config.ts`, `npm run build:mainland`, `FOODMAP_MAINLAND_BASE_PATH` | 已实现，需复验 |
| `MainlandDeploymentVerifier` | Verify local `dist/` and deployed mainland URL without GitHub Pages `/foodMap/` assumptions | `scripts/verify_mainland_deployment.mjs`, `npm run verify:mainland:deployment`, `FOODMAP_MAINLAND_DEPLOY_URL` | 已实现，需远端证据 |
| `EdgeOneDeployAdapter` | Deploy the verified mainland `dist/` package through EdgeOne CLI when API token is available | `scripts/deploy_edgeone_pages.mjs`, `npm run deploy:edgeone`, environment-only API token, `EDGEONE_PROJECT_NAME` | 已实现，需公网验收 |
