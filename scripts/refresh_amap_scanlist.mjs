...
const OUTPUT_PATH = "src/recommendations/amapWuhanScanlist.generated.ts";
...
async function readManualVerifiedIds() {
  try {
    const source = await fs.readFile(MANUAL_VERIFIED_PATH, "utf8");
    return new Set(Array.from(source.matchAll(/poiId:\s*"([^"]+)"/g)).map((match) => match[1]));
  } catch {
    return new Set();
  }
}

function finalVerificationStatus(item, manualVerifiedIds) {
  return manualVerifiedIds.has(item.poiId) ? "verified_manual_overlay" : item.verification.status;
}

function buildReport(recommendations, observedAt, manualVerifiedIds) {
  const verified = recommendations.filter((item) => finalVerificationStatus(item, manualVerifiedIds).startsWith("verified"));
  const conflicts = recommendations.filter((item) => item.verification.status === "conflict");
  const unverified = recommendations.filter((item) => finalVerificationStatus(item, manualVerifiedIds) === "unverified");
  const imageVerified = recommendations.filter((item) => item.imageMatched).length;
  const rows = recommendations.map((item) => {
    const finalStatus = finalVerificationStatus(item, manualVerifiedIds);
    const risk = finalStatus === "verified_manual_overlay" ? "公开地图地点页人工复核通过" : (item.verification.warnings.join("；") || "通过");
    const imageStatus = item.imageMatched ? "图片 alt 与门店名匹配" : "未通过";
    return `| ${item.rank} | ${item.name} | ${item.poiId} | ${finalStatus} | ${item.district} | ${item.address} | ${imageStatus} | ${risk} |`;
  }).join("\n");
  return `# 高德扫街榜刷新报告
...
async function main() {
  ...
  const manualVerifiedIds = await readManualVerifiedIds();
...
`;
  ...
  await fs.writeFile(REPORT_PATH, buildReport(recommendations, observedAt, manualVerifiedIds));
  console.log(`Generated ${OUTPUT_PATH} and ${REPORT_PATH}`);
}
