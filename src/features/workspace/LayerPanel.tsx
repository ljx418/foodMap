    ...
    <aside className="layer-panel" data-testid="layer-panel">
      ...
      <div className={recommendationControl ? "display-summary" : "display-summary display-summary--compact"}>
        <span>
          ...
          <small>个人可见 / 共 {places.length}</small>
        </span>
        {recommendationControl ? (
          <span>
            <strong>{recommendationControl.visible ? recommendationControl.mappable : 0}</strong>
            <small>榜单显示 / 共 {recommendationControl.total}</small>
          </span>
        ) : null}
        <span>
        ...
        <section className="display-section">
          ...
          <label className="display-toggle-card">
            <span className="display-toggle-card__main">
              <span>显示榜单推荐</span>
              <small>
                来源：高德扫街榜。{recommendationControl.mappable} 个可上图，{recommendationControl.approximate} 个建议手动校准
              </small>
            </span>
            <input
              type="checkbox"
      ...
      <div className="display-note">分类管理会在后续版本补齐；当前优先保证地图上“个人收藏”和“榜单推荐”不混淆。</div>
    </aside>
  );
}
